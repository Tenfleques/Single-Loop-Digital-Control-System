...
public class IDigitalRegulator {
    ...
    public IDigitalRegulator(Double [] q, Double [] a, Double b, Double T0, Double eps, Integer d, Integer N){
    ...
    public List<List<Double>> optimizeSettings(){
        ...
        while(j < N){
            for (int i =  max_i; i < N; ++i){
                u[i] = u[i - 1] + q[0] * (yz[i] - y[i]);
                y[i + 1] = a[0]*y[i] + a[1]*y[i - 1] + b*u[i-d];

                dudq[0][i] = dudq[0][i - 1] + (yz[i] - y[i]) - q[0]*dydq[0][i];
            ...
            gradient = Math.sqrt(Math.pow(dSdq[0][0],2.0));
